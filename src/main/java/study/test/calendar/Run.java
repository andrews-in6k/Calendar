...
public class Run {
    ...
    public static void printDayNumbers(int firstDayOfMonthWeekDayId, int lastDayOfMonthWeekDayId, int thisMonthLength, int prevMonthLength) {
        ...
        for (int i = 0; i < calendarSize; i++) {
            ...
            ifEndOfWeek(i);
            ifPrevMonth(checkIfThisMonth, prevMonthLength);
            ifNextMonth(checkIfThisMonth, thisMonthLength);
            ifThisMonth(checkIfThisMonth, thisMonthLength);
        }
    }
    ...
    private static boolean isItHolydayWeekDaysNamer(int iter) {
        return (getDayOfWeekName(iter).equals(DayOfWeek.SUNDAY) || getDayOfWeekName(iter).equals(DayOfWeek.SATURDAY));
    }

    private static boolean isItHolydayDayNumber(int checkIfThisMonth) {
    ...
    private static boolean isItPrevMonth(int checkIfThisMonth) {
        return checkIfThisMonth <= 0;
    }

    private static boolean isItNextMonth(int checkIfThisMonth, int thisMonthLength) {
    ...
    private static void ifPrevMonth(int checkIfThisMonth, int prevMonthLength){
        if (isItPrevMonth(checkIfThisMonth)) {
            printPrevOrNextMonthDayNum(prevMonthLength + checkIfThisMonth);
        }
    }

    private static void ifThisMonth(int checkIfThisMonth, int thisMonthLength){
    ...
    private static void ifNextMonth(int checkIfThisMonth, int thisMonthLength){
        if (isItNextMonth(checkIfThisMonth, thisMonthLength)) {
            printPrevOrNextMonthDayNum(checkIfThisMonth - thisMonthLength);
        }
    }

    private static DayOfWeek getDayOfWeekName(int iter) {
